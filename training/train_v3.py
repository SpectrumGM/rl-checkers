"""
Stage 3: Self-play + спарринг с minimax.
"""
import torch
...
def play_game(model, num_sims=50, opponent=None, alpha_color=1):
    """Играет одну партию. opponent=None значит self-play."""
    ...
    while game.get_winner() is None:
        ...
        is_alpha_turn = (game.current_player == alpha_color) or (opponent is None)
...
def train(iterations=30, games_per_iter=40, epochs=15,
          num_sims=50, batch_size=64, lr=0.0003):
    ...
    for iteration in range(iterations):
        ...
        # Половина self-play, половина vs minimax
        n_self = games_per_iter // 2
        n_vs = games_per_iter - n_self

        for g in tqdm(range(n_self), desc="Self-play"):
            s, p, v, w = play_game(model, num_sims)
            all_s.extend(s); all_p.extend(p); all_v.extend(v)
            wins[w] = wins.get(w, 0) + 1

        mm_wins = {1: 0, -1: 0, 0: 0}
        for g in tqdm(range(n_vs), desc="vs Minimax"):
            color = 1 if g % 2 == 0 else -1
            s, p, v, w = play_game(model, num_sims, opponent=minimax, alpha_color=color)
            all_s.extend(s); all_p.extend(p); all_v.extend(v)
            wins[w] = wins.get(w, 0) + 1
            mm_wins[w] = mm_wins.get(w, 0) + 1

        print(f"Total: W={wins[1]} B={wins[-1]} D={wins[0]} | Pos={len(all_s)}")
        print(f"vs MM: W={mm_wins[1]} B={mm_wins[-1]} D={mm_wins[0]}")
        ...
        torch.save(model.state_dict(), f"models/alpha_v3_iter_{iteration+1}.pth")
...
if __name__ == '__main__':
    train(iterations=30, games_per_iter=40, epochs=15, num_sims=50, batch_size=64, lr=0.0003)
